...
class parser:
    ...
    def get_label_head_with_class(self, class_name):
        return "<span class='" + class_name + "'>"

    def get_label_head_with_color(self, color_value):
        return "<span style='color:#" + color_value + "'>"

    def get_label_tail(self):
        return "</span>"

    def has_element(self, el_array, el):
        for i in el_array:
            if i == el:
                return True
        return False
    ...
    # 文本转化成html
    def parse_style_info(self, equip_desc, DefaultCss=None):
        DefaultCss = DefaultCss if DefaultCss else "#Y"
        result = self.get_label_head_with_class(self.XyqCssSetting[DefaultCss])
        ...
        while i < length:
            ...
            if (equip_desc[i + 1: i + 1 + 1] == "c"):
                result += self.get_label_tail()
                color_start = i + 1 + 1
                color_value = equip_desc[color_start: (color_start + self.ColorLen)]
                result += self.get_label_head_with_color(color_value)
            ...
            if (self.has_element(self.CommonStyleSet, class_label)):
                result += self.get_label_tail()
                result += self.get_label_head_with_class(self.XyqCssSetting[class_label])
                i = i + 1 + 1
                last_common_style["kind"] = self.ClassStyle
                last_common_style["value"] = class_label
                continue
            elif (self.has_element(self.SepicalStyleSet, class_label)):

                if (self.has_element(spcial_class_stack, class_label)):
                    i = i + 1 + 1
                    continue
                result += self.get_label_tail()
                result += self.get_label_head_with_class(self.XyqCssSetting[class_label])
                if (last_common_style["kind"] == self.ClassStyle):
                    result += self.get_label_head_with_class(self.XyqCssSetting[last_common_style["value"]])
                else:
                    result += self.get_label_head_with_color(last_common_style["value"])
    ...
    #人物装备
    def htmlToEquip(self, content , merge_param):
        ...
        # print(html_str)
        html = self.parse_style_info(content)
        #print(html)
        #html = "<span class='equip_desc_yellow'><br>等级 100  五行 土<br><br>伤害 +357 命中 +381<br>耐久度 481<br>锻炼等级 4  镶嵌宝石 太阳石<br></span><span class='equip_desc_green'></span><span class='equip_desc_green'>魔力 +27</span><span class='equip_desc_yellow'></span><span class='equip_desc_yellow'><br></span><span class='equip_desc_white'>制造者：★000爆爆0O0强化打造</span><span class='equip_desc_yellow'>  </span>"
        ...
        equip['price_fen'] = equip.get("price",0)*100
        print(equip)
        return equip
    ...
    #召唤兽装备
    def htmlToPetEquip(self, content, merge_param={}):
        html = self.parse_style_info(content)
        html = etree.HTML(html)
        #print(html)
        it = html.xpath("//span[@class='equip_desc_yellow']/text()")
        ...
        if petequip.get("price",0) != 0:
            petequip['price_fen'] = petequip.get("price",0) * 100
        return  petequip

    #灵饰
    def htmlToLingShi(self,content,merge_params={}):
        # var equip = {
        ...
        lingshi['price_fen'] = int(lingshi.get("price",0))
        #print(lingshi)
        return lingshi
...
# 特效，特技，套装
#desc = "#r等级 70  五行 水#r#r防御 +190#r耐久度 449#r锻炼等级 7  镶嵌宝石 月亮石#r#G#G力量 +16#Y #G耐力 -4#Y#r#c4DBAF4特技：#c4DBAF4水清诀#Y#Y#r#c4DBAF4特效：#c4DBAF4简易#Y#r#c4DBAF4套装效果：追加法术满天花雨#Y#Y#r#G开运孔数：3孔/3孔#G#r符石: 伤害 +1.5#n#G#r符石: 伤害 +1.5#n#G#r符石: 力量 +1#n#G#r星位：防御 +5#n#r#cEE82EE符石组合: 无心插柳#r门派条件：无#r部位条件：无#r普通物理攻击时会造成溅射效果，对另外两个目标造成所受伤害20%的伤害，仅对NPC使用时有效。#Y#r#W制造者：＂碧魂．#Y#r#Y熔炼效果：#r#Y#r+3耐力 +9防御 #r#Y "
desc = "#r等级 130  五行 金#r#r防御 +73 魔法 +142#r耐久度 88  修理失败 2次#r锻炼等级 6  镶嵌宝石 红玛瑙#r#G#G命中 +150#Y#Y#r#G开运孔数：4孔/5孔#G#r符石: 伤害 +1.5#n#G#r符石: 伤害 +1.5#n#G#r符石: 力量 +1#n#G#r符石: 力量 +1 法术防御 +2#n#r#cEE82EE符石组合: 无心插柳#r门派条件：无#r部位条件：无#r普通物理攻击时会造成溅射效果，对另外两个目标造成所受伤害25%的伤害，仅对NPC使用时有效。#Y#r#W制造者：《心舞飞扬》强化打造#Y "
my.htmlToEquip(desc,{})
# 多特效
